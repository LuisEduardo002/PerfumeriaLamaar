...
export default function ProductGrid({ products = [], loading = false, emptyMessage = 'No se encontraron perfumes que coincidan con tu búsqueda.' }) {
  ...
  if (products.length === 0) {
    return <div className="my-4 rounded-3xl border border-stone-200 bg-white p-8 py-16 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#F6F0E3] text-[#4B1E28]"><PackageSearch className="h-8 w-8" /></div><h3 className="mb-2 font-serif text-xl font-bold text-[#111111]">Sin resultados</h3><p className="mx-auto max-w-md text-sm text-slate-500">{emptyMessage}</p></div>;
  }
